...
class SpecialKeyAddon() : DialogFragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        dialogView.dialog_SpecialkeyAddonSelect_rg.check(when ( checkedFunction ) {
        ...
        dialogView.dialog_SpecialkeyAddonSelect_ok.setOnClickListener {
            checkedFunction = when ( dialog_SpecialkeyAddonSelect_rg.checkedRadioButtonId ) {
            ...
            activity?.findViewById<Button>(R.id.setting_specialkeyAddon)?.text = when ( checkedFunction ) {
                0 -> "없음"
                1 -> "상용구"
                2 -> "커서이동 및 편집"
                else -> "없음"
            ...
            activity?.getSharedPreferences("keyboardSetting", Context.MODE_PRIVATE)?.edit()?.putInt("KeyboardSpecialkeyAddon", checkedFunction)?.apply()
        }
